...
var action_register = function (current_id, action_name, current_audio_time) {
  $.post("/transcription/action/", {job_id:$('#job').attr('job_id'),transcription_id:current_id,audio_time:current_audio_time,action_name:action_name,csrfmiddlewaretoken:'<input type='hidden' name='csrfmiddlewaretoken' value='8qTYqfbq0ad8iLXnNah3EQP3hNUS594C' />'})
  .done(function(revision_id) {
    if (action_name=='tick') { //revision complete
      ...
      if (utterance.length!=0) {
        $.post("/transcription/revision/", {revision_id:revision_id,utterance:utterance.join(' ')});
      }
    }
  });
}
